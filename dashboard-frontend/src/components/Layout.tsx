...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // Auto-ouvre la cat de la route active + ferme la sidebar sur mobile
  useEffect(() => {
    ...
    <div className="flex h-screen overflow-hidden">
      ...
      <aside
        className={`
          ...
          w-64 md:w-56 flex flex-col shrink-0 overflow-hidden
        ...
        <div className="flex items-center justify-between px-4 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
          ...
          <button
            className="md:hidden p-1.5 rounded hover:bg-white/10 transition"
        ...
        <div className="mx-3 my-2 px-3 py-2 rounded-lg flex items-center gap-2"
          ...
          <div className={`w-2 h-2 rounded-full shrink-0 ${serverStatus ? 'animate-pulse' : ''}`}
        ...
        <div style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <div className="px-3 pb-3 flex items-center justify-between">
            ...
            <button onClick={() => { logout(); navigate('/login') }}
                    className="text-xs px-2 py-1 rounded hover:bg-white/10 transition shrink-0"
                    style={{ color: 'var(--text-muted)' }}>
              Déco
            </button>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        {/* Topbar */}
        <header className="shrink-0 flex items-center justify-between px-4 md:px-6 py-3 gap-3"
        ...
        {/* Bannière VIEWER */}
        {isViewer && (
          <div className="shrink-0 flex items-center gap-2 px-4 md:px-5 py-2 text-xs font-medium"
...
function Breadcrumb() {
  ...
  if (!match) return <span className="text-sm font-semibold truncate" style={{ color: 'var(--text)' }}>SunGuard</span>
  return (
    <div className="flex items-center gap-2 text-sm min-w-0">
      <span className="hidden sm:block shrink-0" style={{ color: 'var(--text-muted)' }}>{match.cat}</span>
      <span className="hidden sm:block shrink-0" style={{ color: 'var(--border)' }}>/</span>
      <span className="font-semibold truncate" style={{ color: 'var(--text)' }}>{match.icon} {match.label}</span>
    </div>
  )
}

...
// Sun positions per variant (right side of horizon for dawn/dusk, top center for noon)
...
export default function SunSky({
  ...
  children,
  className = '',
}: { variant?: SkyVariant; children?: ReactNode; className?: string }) {
...
// ── Pre-computed star pattern (deterministic — no flicker on re-render) ─────────
...
function Stars() {
  const stars = useMemo(() =>
    STAR_SEED.map(([x, y, brightness], i) => ({
      x, y, brightness,
      size: brightness > 0.7 ? 1.5 : 1,
      // Stars fade as they get closer to horizon
...
// ── Mountain SVG layers ─────────────────────────────────────────────────────────
// Stylised silhouettes with subtle voxel-stepped peaks (Minecraft hint, refined).

function MountainsFar({ fill }: { fill: string }) {
  return (
...
function MountainsMid({ fill }: { fill: string }) {
  return (
...
// ── Floating voxel island (subtle Minecraft hint) ───────────────────────────────
